...
As mentioned in section (\ref{backg:annealing}), this model is an extension of Binary Quadratic Models (BQM) or Quadratic Unconstrained Binary Optimization model (QUBO).

\subsubsection{Map Variables}
\label{approach:annealing.prepare.map}
...
\begin{align}
  m:
  \quad
  \mathbb{I} \times \mathbb{T} \to \mathbb{M}:
  \quad
...
\subsubsection{Discretize Power Levels}
\label{approach:annealing.prepare.discretize}
...
\subsubsection{Rewrite Objective Function}
...
\subsubsection{Rewrite Startup and Shutdown Costs}
...
\begin{align}
  ...
  v_{\text{DQM}} = & \qquad
  |\mathbb{T}| \cdot |\mathbb{I}|
  \\
  \label{formula:dqm.num.quadratic.biases}
  q_{\text{DQM}} = & \qquad
  \left( |\mathbb{T}| - 1 \right) \cdot |\mathbb{I}|
  + |\mathbb{T}| \cdot \frac{|\mathbb{I}| \cdot \left( |\mathbb{I}| - 1 \right)}{2}
\end{align}
\end{subequations}
